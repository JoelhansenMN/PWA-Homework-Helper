...
export const getDb = async () => {
  ...
  const open = await openDB(jatedb, 1);
  const text = open.transaction(jatedb, 'readonly');
  const store = text.objectStore(jatedb);
